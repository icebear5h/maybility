...
const usePlacing = (events: Event[], date: DateValue) => {
  return React.useMemo(() => {
    const atomCount = (24 * 60) / 30; // 24 hours * 60 minutes / 30-minute intervals
    ...
    events.forEach(({ id, startTime: start, endTime: end }) => {
      ...
      const startAtom = Math.floor(((startDateTime.getTime() - startTime) / (24 * 60 * 60 * 1000)) * atomCount);
      const endAtom = Math.ceil(((endDateTime.getTime() - startTime) / (24 * 60 * 60 * 1000)) * atomCount);
    ...
    <div ref={setNodeRef} className="flex-1 flex flex-col relative">
      ...
      >
        {Array.from({ length: 24 }, (_, index) => (
          <span key={index} className="border-b border-gray-200 dark:border-neutral-700 flex-1" />
        ))}
      </Link>
